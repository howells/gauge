...
const readOutput = (paginated: boolean): CommandSpec["output"] => ({
  paginated,
  supportsFields: true,
  supportsNdjson: true,
});

const safety = (dryRun: boolean): CommandSpec["safety"] => ({
  dryRun,
  sanitizesRemoteStrings: true,
});
...
export const renderCommandExamplesMarkdown = (): string =>
  [
