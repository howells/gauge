...
const lstatIfPresent = (target: string): fs.Stats | null => {
  try {
...
const hasSafeCodexAuth = (homePath: string): boolean => {
  try {
...
const pass = (id: string, message: string): DoctorCheck => ({
  id,
  message,
  status: "pass",
});

const warning = (id: string, message: string): DoctorCheck => ({
  id,
  message,
  status: "warning",
});
...
const fail = (id: string, message: string): DoctorCheck => ({
  id,
  message,
  status: "fail",
});

const inspectCredentialFile = (
  filePath: string,
  id: string,
  label: string,
  required: boolean,
...
const inspectDataRoot = (dataRoot: string, checks: DoctorCheck[]): void => {
